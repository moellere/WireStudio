...
| Serial monitor + UDP logging | Post-flash boot verification: the flash dialog relays the slot's recorder after a flash and shows the bench's boot verdict; assert per-framework boot markers (ESPHome boot log, Meshtastic banner, CIRCUITPY enumeration, LoRaWAN join) | Upgrades "flashed" to "flashed and booted" — the failure class CI cannot see |
